...
門檻 `SEMANTIC_STANDOUT = 0.9` 是量出來的，不是猜的。對真實卡冊跑 30 個查詢（18 個答得出、12 個答不出）：
...
0.9 落在那段空隙裡、偏下緣——**搜不到自己寫過的卡，比多看到一張卡糟糕得多**，所以疑慮花在留下結果而不是砍掉結果。修正後那 30 題是 0 漏接、0 誤中。

`null` 代表「沒有意見」（卡片太少、或卡冊裡的卡彼此都差不多），此時不過濾。改動這個數字前先重跑那組查詢，不要憑感覺調。
...
| 重複判定 | 只看標題相同與**用字重疊**（字元 bigram，門檻 0.5）。不要把相似度加回去：卡冊少的時候 §1.3 的相對分佈就是雜訊——實測四張卡時，重打一次的卡片只拿到 0.29，兩張不相干的卡片卻是 1.00 | 原本「raw cosine ≥ 0.9」在使用者的卡冊上把 3 組只是相關的卡片（Attention ↔ QKV）判成重複；同一批卡片的用字重疊最高只有 0.092 |
