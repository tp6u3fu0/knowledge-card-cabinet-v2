    ...
    with get_connection() as connection:
        row = connection.execute(
            ...
            INSERT INTO cards (
                ...
                source, tags, status, embedding, embedding_model, updated_at
            )
            VALUES (
                %(id)s, %(number)s, %(topic)s, %(title)s, %(question)s, %(summary)s,
                %(analogy)s, %(detail)s, %(source)s, %(tags)s, %(status)s,
    ...
    with get_connection() as connection:
        rows = connection.execute(
            ...
            SELECT id, number, topic, title, question, summary, analogy, detail,
                   source, tags, status, embedding_model, created_at, updated_at,
...
def list_cards() -> list[dict[str, Any]]:
    with get_connection() as connection:
        return [dict(row) for row in connection.execute("SELECT * FROM cards ORDER BY number").fetchall()]


def get_card(card_id: str) -> dict[str, Any] | None:
    with get_connection() as connection:
        row = connection.execute("SELECT * FROM cards WHERE id = %s", (card_id,)).fetchone()
    ...
    with get_connection() as connection:
        rows = connection.execute(
            ...
            SELECT r.relation_type, r.score, r.status,
                   ...
                   c.analogy, c.detail, c.source, c.tags, c.status AS card_status,
