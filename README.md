...
新增卡片時只需要填四個欄位：標題、想回答的問題、一句話摘要、分類。比喻、細節、來源、標籤、主題與編號收在「展開進階欄位」裡，隨時可以補。卡片 id 與編號（`KC-000123`）由 runtime 產生，指派後不會再變。填了來源連結的話，卡片背面的來源就會變成一個連結，點下去在瀏覽器開啟原始資料；連結種類（網頁、Notion…）由 runtime 從網址判斷，只接受 http/https。也可以貼上筆記，讓摘要模型產生標題、問題、摘要與標籤草稿，確認後再寫入資料庫。embedding 會在新增或編輯卡片後重新建立，並更新語意關聯；封面綁定卡片 id，編輯不會改變它。
