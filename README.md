...
- 關聯圖支援分類聚焦、強關聯篩選、依顏色分道排列、關聯來源／原因／分數與自製關聯移除
- 搜尋支援分類、標籤與排序篩選，並顯示命中原因；文字比對與語意比對是兩條獨立管線，模型壞掉時文字搜尋照常
- 全域快速搜尋：在任何應用程式裡按 `Ctrl/Cmd + Shift + K` 叫出疊層，不必先開卡冊視窗
- 標籤拼法與分類在儲存時自動整理；疑似重複的卡片會自己浮上來
- 為每張卡自動產生獨一無二的抽象封面（綁定卡片 id，編輯內容或更換模型都不會改變它），同一分類的卡片使用同一個顏色
- 卡片垃圾桶、還原與永久刪除
- 資料庫 JSON 匯出、匯入與重置
- 本機模型下載、切換與快取管理
...
配對流程：桌面端產生一次性配對碼（十分鐘失效，只能換一把鑰匙），手機掃描 QR code 或手動輸入。手機拿到的是自己的長期權杖，桌面端不顯示也不保存它，隨時可以單獨撤銷。憑證是自簽的，**手機端必須比對畫面上的 SHA-256 指紋**。

裝置權杖的權限比這台電腦自己的小：卡片、分類、搜尋與垃圾桶可以完整讀寫；主機目前用哪個模型、背景工作跑到哪、主機是哪一版**只能看不能改**；換模型、開關區網分享、管理其他裝置、匯入匯出與重置資料庫則一律拒絕。所以一支被撿走的手機能看到你的卡片，但改不動這台電腦，也帶不走整個資料庫。
...
新增卡片時只需要填四個欄位：標題、想回答的問題、一句話摘要、分類。比喻、細節、來源、標籤、主題與編號收在「展開進階欄位」裡，隨時可以補。卡片 id 與編號（`KC-000123`）由 runtime 產生，指派後不會再變。填了來源連結的話，卡片背面的來源就會變成一個連結，點下去在瀏覽器開啟原始資料；連結種類（網頁、Notion…）由 runtime 從網址判斷，只接受 http/https。卡片背面也有「看看來源有沒有變」，按下去才會去讀那一個網址並比對，發現變了會提示這張卡可能需要重新整理——**永遠不會自動改寫卡片內容**。這是 app 除了更新檢查以外唯一的對外請求，`KCC_SOURCE_CHECK=off` 可以完全關掉，配對過的手機不能觸發它。
...
「疑似重複」只看兩件事：標題正規化後相同，或兩張卡的**用字重疊超過一半**（字元 bigram）。刻意不看語意相似度——同一個主題用不同說法寫兩次是關聯，不是重複，關聯圖已經畫出來了；而在卡片還少的時候，相似度的分佈本身就是雜訊。
...
可管理的高層分類；同分類會提高關聯候選的輔助分數，但不會取代 embedding 關聯。並限制每張卡片保留的
語意關聯數量。

搜尋是**兩條各自獨立的管線**：文字比對看得到每一張卡，語意比對只看得到向量寬度相符的卡，兩者的分數合併後才排名（標題 > 問題 > 摘要 > 分類／標籤 > 內文，標題完全相同另外加分）。這代表**模型沒下載、壞掉或正在重建時，文字搜尋照常運作**——AI 是搜尋的增強層，不是搜尋能不能用的前提。中文查詢以字元 bigram 比對，所以「共識為什麼要過半數」找得到「為什麼共識需要過半數？」，即使兩者沒有一個相同的詞。回應會同時附上 `lexical_score` 與 `semantic_score`（沒有可比較向量時為 `null`），方便調權重與除錯；介面只顯示命中原因，不顯示分數。

有查詢時，收藏頁會換成結果列而不是分類牆：每一列直接給標題、問題、**一句話摘要**、分類與命中原因，不必點開卡片就能重新想起那個概念。卡片太少而分不出哪張真的相關時，會直接說出來，而不是替每一列編一個理由。

桌面版另外有一個全域快速搜尋疊層（預設 `Ctrl/Cmd + Shift + K`，搶不到就退到 `Ctrl/Cmd + Alt + K`，都搶不到時介面不會顯示快捷鍵）。它是 `GET /search` 的另一個 client，不是另一套搜尋：`↑`/`↓` 選、`Enter` 展開細節、`Cmd/Ctrl + Enter` 丟到卡冊視窗打開、`Esc` 關閉。視窗開機時就建好並藏著，所以叫出來是即時的。

搜尋的語意分數同樣是相對值——相對於該次查詢自己的分數分布，因此「語意相似」這個標記在同一批結果之間才有區辨力；結果太少而測不出分布時就不標記。關聯建議會把語意分數與分類／主題／標籤命中混合計算。語意分數是**相對於這個收藏本身的相似度分布**測量的，不是絕對 cosine——強多語言模型會把所有配對壓在很窄的高分帶裡，絕對門檻無法分辨「相關」與「兩者都是文字」。
